...
ProfitResult Deposit::getProfit()
{
    ...
    ProfitResult result(templ.getName(), open_date,start_sum.getValute(),start_sum.getValue()+profit.getValue(),profit.getValue(),rates,(templ.getRates().get(start_sum, day_count, capitalization)));
    return result;
}
